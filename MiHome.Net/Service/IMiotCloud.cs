...
public class MIotCloud : IMiotCloud
{
        ...
        if (result.HasText() && result.StartsWith(startValue))
        {
            ...
            if (loginUrlResultString.HasText() && loginUrlResultString.StartsWith(startValue))
            {
                ...
                if (qrCodeLoginResultString.HasText() && qrCodeLoginResultString.StartsWith(startValue))
                {
                    ...
                    //获取cookie过期时间
                    var allCookies = fegiFeignUnitOfWork.GetCookies("https://c3.lp.account.xiaomi.com/lp");
                    var endDates = new List<DateTime>();
                    foreach (var cookie in allCookies)
                    {
                        if (cookie.Name.EndsWith("GMT"))
                        {
                            var tempDate = ParseGmtTime(cookie.Name);
                            endDates.Add(tempDate);
                        }
                    }
    ...
    private DateTime ParseGmtTime(string gmtTimeStr)
    {
        string format = "dd-MMM-yyyy HH:mm:ss 'GMT'";
    ...
    public async Task LoginAsync()
    {
        if (File.Exists(GetAuthFilePath))
        {
            ...
            var diffTime = (tempLoginInfoDto.ExpireTime - DateTime.UtcNow);
        ...
        else
        {
            ...
            await cache.SetValueWithAbsoluteAsync("loginInfo", loginInfoDto, (loginInfoDto.ExpireTime - DateTime.Now).Add(TimeSpan.FromMinutes(-60)));
